...
# LangSmith
client = Client()
tracer = LangChainTracer(project_name="LATMO")
...
@cl.on_chat_start
async def start():
    # Initialize all your existing tools and agents here
    # For example:
    lazy_gmail_toolkit = LazyGmailToolkit()
    local_rag_tool = LocalRAGTool()
    google_calendar_toolkit = GoogleCalendarToolkit()
    ...
    cl.user_session.set("agent", prioritized_agent)

@cl.on_message
async def main(message: cl.Message):
    agent = cl.user_session.get("agent")  # type: PrioritizedMemoryAgent
    current_datetime = get_current_datetime()
    
    res = await cl.make_async(agent.runnable)(f"{message.content} (Current date and time: {current_datetime})")
